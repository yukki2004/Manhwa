...
{
    public class HideCommentCommandHandler : IRequestHandler<HideCommentCommand, HideCommentResponse>
    {
        ...
        public async Task<HideCommentResponse> Handle(HideCommentCommand request, CancellationToken ct)
        {
            ...
            {
                throw new BusinessRuleViolationException("Bình luận này đã bị ẩn trước đó.", "COMMENT_ALREADY_HIDDEN");
            }
            ...
            if (story.IsPublish == StoryPublishStatus.Deleted && !isAdmin)
                throw new BusinessRuleViolationException("Truyện đã bị xóa, không thể ẩn bình luận", "STORY_DELETED");

            if (story.IsPublish == StoryPublishStatus.Hidden && !isAdmin && !isStoryOwner)
                throw new BusinessRuleViolationException("Truyện đang ẩn, bạn không có quyền thao tác", "STORY_HIDDEN");
            ...
            return new HideCommentResponse { Success = true, Message = "Bình luận đã được ẩn." };
        }
    }
}